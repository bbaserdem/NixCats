...
if require('nixCatsUtils').isNixCats then
  ...
  if this_fdir ~= '' then
    this_nixpkgs = '(builtins.getFlake "' ..
      this_fdir .. '").inputs.nixpkgs'
    if this_host ~= '' then
      this_nixos = '(builtins.getFlake "' .. this_fdir ..
        '").nixosConfigurations."' .. this_host .. '".options'
      if this_user ~= '' then
        this_homeManager = '(builtins.getFlake "' .. this_fdir ..
          '").homeConfigurations."' .. this_user .. '@' .. this_host .. '".options'
      end
    end
  end
end
